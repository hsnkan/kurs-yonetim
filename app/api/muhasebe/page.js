...
export default function MaliYonetimPage() {
  ...
  const maliRaporuGetir = async () => {
    ...
    try {
      ...
      if (result.success) setRapor(result.data);
    } catch (err) {
  ...
  const toplamYillikKazanc = rapor.reduce(
    (acc, item) => acc + item.toplamKazanc,
    0,
  );
  const toplamYeniKayit = rapor.reduce((acc, item) => acc + item.yeniKayit, 0);
  const toplamAyrilan = rapor.reduce((acc, item) => acc + item.ayrilan, 0);
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto font-sans">
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-lg border border-slate-300">
        ...
        {loading ? (
          <p className="text-slate-700 font-bold">Raporlar hesaplanıyor...</p>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {rapor.map((item, idx) => (
                  ...
                  >
                    <td className="p-4 font-black text-base">{item.ay}</td>
                    <td className="p-4 font-bold text-blue-700">
                      +{item.yeniKayit} Öğrenci
                    </td>
                    <td className="p-4 font-bold text-rose-600">
                      -{item.ayrilan} Öğrenci
                    </td>
                    <td className="p-4 text-right font-black text-emerald-700 text-base">
                      ₺ {item.toplamKazanc.toLocaleString("tr-TR")}
                    </td>
                  </tr>
                ))}
              </tbody>
