...
export default function OgrencilerPage() {
  ...
  const [grupLink, setGrupLink] = useState('https://chat.whatsapp.com/Kx1Y2z3abc456def...');
  ...
  const ogrencileriGetir = async () => {
    try {
      const res = await fetch('/api/ogrenciler?durum=AKTIF', { cache: 'no-store' });
  ...
  // TELEFON NFC İLE KAYIT ESNASINDA KART OKUTMA (Android / Chrome)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (confirm(`1. Veli (${yeniOgrenci.veliAdSoyad}) için WhatsApp grup daveti gönderilsin mi?`)) {
          whatsappDavetGonder(yeniOgrenci.veliAdSoyad, yeniOgrenci.veliTelefon);
        }

        if (
          yeniOgrenci.ikinciVeliTelefon &&
          confirm(`2. Veli (${yeniOgrenci.ikinciVeliAdSoyad}) için WhatsApp grup daveti gönderilsin mi?`)
        ) {
          whatsappDavetGonder(yeniOgrenci.ikinciVeliAdSoyad, yeniOgrenci.ikinciVeliTelefon);
        }
  ...
  const ogrenciArsivle = async (id, adSoyad) => {
    if (!confirm(`${adSoyad} isimli öğrenciyi arşivlemek istediğinize emin misiniz?`)) return;
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto font-sans bg-slate-100 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <h1 className="text-3xl font-black text-slate-950 tracking-tight">🎓 Aktif Öğrenci Yönetimi</h1>
        <Link
          href="/ogrenciler/arsiv"
          className="bg-slate-800 hover:bg-slate-900 text-white font-bold py-2.5 px-5 rounded-xl text-sm transition shadow-md self-start md:self-auto"
        >
          📁 Ayrılan / Pasif Öğrenci Arşivi →
        </Link>
      </div>
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-lg border border-slate-300 mb-8">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4 bg-blue-50/60 p-4 rounded-2xl border border-blue-200">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1">Yakınlık</label>
              <select
                ...
                onChange={(e) => setYeniOgrenci({ ...yeniOgrenci, veliYakinlik: e.target.value })}
              >
            ...
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1">Ad Soyad *</label>
              <input
                type="text"
                required
          ...
          <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-300">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setYeniOgrenci({ ...yeniOgrenci, ikinciVeliYakinlik: e.target.value })}
              >
                <option value="Baba">Baba</option>
                <option value="Anne">Anne</option>
                <option value="Vasi">Vasi / Diğer</option>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setYeniOgrenci({ ...yeniOgrenci, ikinciVeliAdSoyad: e.target.value })}
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1">Telefon</label>
              <input
                ...
                onChange={(e) => setYeniOgrenci({ ...yeniOgrenci, ikinciVeliTelefon: e.target.value })}
              />
            </div>
          </div>

          {/* NFC KART TANITMA ALANI (USB + TELEFON UYUMLU) */}
          <div>
            <label className="block text-xs font-black text-slate-900 uppercase tracking-wider mb-1.5">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) => setYeniOgrenci({ ...yeniOgrenci, nfcUid: e.target.value })}
              />
              ...
              {nfcDestegi && (
                ...
                >
                  <span>📱</span>
                  <span>{nfcOkunuyor ? 'Kartı Yaklaştır...' : 'Telefonla Oku'}</span>
                </button>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setYeniOgrenci({ ...yeniOgrenci, odemeGunu: Number(e.target.value) })}
            />
          </div>
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl shadow-lg border border-slate-300">
        ...
        {loading ? (
          ...
          <p className="text-slate-700 font-bold py-4">Henüz aktif kayıtlı öğrenci yok.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {ogrenciler.map((o) => (
                  <tr key={o._id} className="border-b border-slate-200 hover:bg-slate-50 text-slate-950 transition">
                    <td className="p-4 font-black text-base">{o.adSoyad}</td>
                    <td className="p-4">
                      <div className="font-extrabold text-slate-900">
                        {o.veliAdSoyad} <span className="text-xs text-blue-700">({o.veliYakinlik || 'Anne'})</span>
                      </div>
                      <div className="text-xs font-bold text-slate-700 font-mono mt-0.5">{o.veliTelefon}</div>
                    </td>
                    <td className="p-4">
                      {o.ikinciVeliAdSoyad ? (
                        <>
                          <div className="font-extrabold text-slate-900">
                            {o.ikinciVeliAdSoyad}{' '}
                            <span className="text-xs text-slate-600">({o.ikinciVeliYakinlik || 'Baba'})</span>
                          </div>
                        ...
                        <span className="text-xs text-slate-400 font-semibold">Kayıtlı Değil</span>
                      )}
                    </td>
